...
__global__ void update_optimizer_intermediates(
	float* fastEMA,
	float* slowEMA,
	float* variance,
	float* grad,
    ...
    float averagedGrad = grad[index] / (float) batchSize;

    fastEMA[index] = BETA1_ * fastEMA[index] + (1.0f - BETA1_) * averagedGrad;
    slowEMA[index] = BETA3_ * slowEMA[index] + (1.0f - BETA3_) * averagedGrad;
  	variance[index] = BETA2_ * variance[index] + (1.0f - BETA2_) * averagedGrad * averagedGrad;
}

// iterationNum starts with 1
__global__ void update_weights_per_adeamix(
	float* weight, 
	float* fastEMA, 
	float* slowEMA, 
	float* variance, 
	float* beta1_pow_store, 
	float* beta2_pow_store, 
	float* beta3_pow_store, 
	float learningRate, 
	int iterationNum, 
	float ALPHA_,
	float WEIGHT_DECAY_,
    ...
    float momentum_final = fastEMABiased + ALPHA_ * slowEMA[index]; // slowEMABiased;
    ...
    if (weightUpdate < 0.01f && weightUpdate > -0.01f) {
		weight[index] = weight[index] - weightUpdate - learningRate * WEIGHT_DECAY_ * weight[index];
    } else {
		weight[index] = weight[index] - (weightUpdate / fabsf(weightUpdate / 0.01f)) - learningRate * WEIGHT_DECAY_ * weight[index];
    }
}
...
__global__ void update_weights_per_adeamix_decay_relative_to_one(float* weight, float* fastEMA, float* slowEMA, float* variance, float* beta1_pow_store, float* beta2_pow_store, float* beta3_pow_store, float learningRate, int iterationNum, float ALPHA_, float WEIGHT_DECAY_, int size) {
    ...
    float weightUpdate = learningRate * momentum_variance_scaled_final;
    if (weightUpdate < 0.01f && weightUpdate > -0.01f) {
		weight[index] = weight[index] - weightUpdate - learningRate * WEIGHT_DECAY_ * (weight[index] - 1.0f);
    } else {
		weight[index] = weight[index] - (weightUpdate / fabsf(weightUpdate / 0.01f)) - learningRate * WEIGHT_DECAY_ * (weight[index] - 1.0f);
    }
}
...
__global__ void update_weights_per_adeamix_no_decay(float* weight, float* fastEMA, float* slowEMA, float* variance, float* beta1_pow_store, float* beta2_pow_store, float* beta3_pow_store, float learningRate, int iterationNum, float ALPHA_, int size) {
    ...
    float weightUpdate = learningRate * momentum_variance_scaled_final;
    if (weightUpdate < 0.01f && weightUpdate > -0.01f) {
		weight[index] = weight[index] - weightUpdate;
    } else {
		weight[index] = weight[index] - (weightUpdate / fabsf(weightUpdate / 0.01f));
    }
}
...
void apply_adeamix_optimizer(int iterationNum, float learningRate) {
	...
	update_optimizer_intermediates<<<numBlocks, threadsPerBlock>>>(fastEMA_embedding_weights, slowEMA_embedding_weights, variance_embedding_weights, gradientAccumulation_embedding_weights, ADEAMIX_BETA1, ADEAMIX_BETA2, ADEAMIX_BETA3, TRAINING_BATCH_SIZE, xTotalThreads);
	update_weights_per_adeamix_no_decay<<<numBlocks, threadsPerBlock>>>(embedding_weights_DEVICE, fastEMA_embedding_weights, slowEMA_embedding_weights, variance_embedding_weights, beta1_pow_store, beta2_pow_store, beta3_pow_store, learningRate, iterationNum, ADEAMIX_ALPHA, xTotalThreads);
	...
	xTotalThreads = dim;
	numBlocks = (xTotalThreads + threadsPerBlock - 1) / threadsPerBlock;
	update_optimizer_intermediates<<<numBlocks, threadsPerBlock>>>(fastEMA_final_RMS_gamma_weights, slowEMA_final_RMS_gamma_weights, variance_final_RMS_gamma_weights, gradientAccumulation_final_RMS_gamma_weights, ADEAMIX_BETA1, ADEAMIX_BETA2, ADEAMIX_BETA3, TRAINING_BATCH_SIZE, xTotalThreads);
	...
	for (int t = 0; t < transformers; t++) {
		...
		numBlocks = (xTotalThreads + threadsPerBlock - 1) / threadsPerBlock;
		update_optimizer_intermediates<<<numBlocks, threadsPerBlock>>>(fastEMA[t].ffn_left_weights, slowEMA[t].ffn_left_weights, variance[t].ffn_left_weights, gradientAccumulation[t].ffn_left_weights, ADEAMIX_BETA1, ADEAMIX_BETA2, ADEAMIX_BETA3, TRAINING_BATCH_SIZE, xTotalThreads);
		...
		numBlocks = (xTotalThreads + threadsPerBlock - 1) / threadsPerBlock;
		update_optimizer_intermediates<<<numBlocks, threadsPerBlock>>>(fastEMA[t].ffn_right_1_weights, slowEMA[t].ffn_right_1_weights, variance[t].ffn_right_1_weights, gradientAccumulation[t].ffn_right_1_weights, ADEAMIX_BETA1, ADEAMIX_BETA2, ADEAMIX_BETA3, TRAINING_BATCH_SIZE, xTotalThreads);
		...
		update_optimizer_intermediates<<<numBlocks, threadsPerBlock>>>(fastEMA[t].ffn_right_2_weights, slowEMA[t].ffn_right_2_weights, variance[t].ffn_right_2_weights, gradientAccumulation[t].ffn_right_2_weights, ADEAMIX_BETA1, ADEAMIX_BETA2, ADEAMIX_BETA3, TRAINING_BATCH_SIZE, xTotalThreads);
		...
		xTotalThreads = dim;
		numBlocks = (xTotalThreads + threadsPerBlock - 1) / threadsPerBlock;
		update_optimizer_intermediates<<<numBlocks, threadsPerBlock>>>(fastEMA[t].rms2_gamma_weights, slowEMA[t].rms2_gamma_weights, variance[t].rms2_gamma_weights, gradientAccumulation[t].rms2_gamma_weights, ADEAMIX_BETA1, ADEAMIX_BETA2, ADEAMIX_BETA3, TRAINING_BATCH_SIZE, xTotalThreads);
		...
		numBlocks = (xTotalThreads + threadsPerBlock - 1) / threadsPerBlock;
		update_optimizer_intermediates<<<numBlocks, threadsPerBlock>>>(fastEMA[t].output_proj_weights, slowEMA[t].output_proj_weights, variance[t].output_proj_weights, gradientAccumulation[t].output_proj_weights, ADEAMIX_BETA1, ADEAMIX_BETA2, ADEAMIX_BETA3, TRAINING_BATCH_SIZE, xTotalThreads);
		...
		update_optimizer_intermediates<<<numBlocks, threadsPerBlock>>>(fastEMA[t].value_weights, slowEMA[t].value_weights, variance[t].value_weights, gradientAccumulation[t].value_weights, ADEAMIX_BETA1, ADEAMIX_BETA2, ADEAMIX_BETA3, TRAINING_BATCH_SIZE, xTotalThreads);
		...
		update_optimizer_intermediates<<<numBlocks, threadsPerBlock>>>(fastEMA[t].query_weights, slowEMA[t].query_weights, variance[t].query_weights, gradientAccumulation[t].query_weights, ADEAMIX_BETA1, ADEAMIX_BETA2, ADEAMIX_BETA3, TRAINING_BATCH_SIZE, xTotalThreads);
		...
		update_optimizer_intermediates<<<numBlocks, threadsPerBlock>>>(fastEMA[t].key_weights, slowEMA[t].key_weights, variance[t].key_weights, gradientAccumulation[t].key_weights, ADEAMIX_BETA1, ADEAMIX_BETA2, ADEAMIX_BETA3, TRAINING_BATCH_SIZE, xTotalThreads);
		...
		xTotalThreads = dim;
		numBlocks = (xTotalThreads + threadsPerBlock - 1) / threadsPerBlock;
		update_optimizer_intermediates<<<numBlocks, threadsPerBlock>>>(fastEMA[t].rms1_gamma_weights, slowEMA[t].rms1_gamma_weights, variance[t].rms1_gamma_weights, gradientAccumulation[t].rms1_gamma_weights, ADEAMIX_BETA1, ADEAMIX_BETA2, ADEAMIX_BETA3, TRAINING_BATCH_SIZE, xTotalThreads);
        ...
        if (CONFIG_QK_RMS_NORM) {
			// Query RMS Gamma Weights (dim)
			update_optimizer_intermediates<<<numBlocks, threadsPerBlock>>>(fastEMA[t].query_gamma_weights, slowEMA[t].query_gamma_weights, variance[t].query_gamma_weights, gradientAccumulation[t].query_gamma_weights, ADEAMIX_BETA1, ADEAMIX_BETA2, ADEAMIX_BETA3, TRAINING_BATCH_SIZE, xTotalThreads);
			update_weights_per_adeamix_decay_relative_to_one<<<numBlocks, threadsPerBlock>>>(transformerWeights_DEVICE[t].query_RMS_weights, fastEMA[t].query_gamma_weights, slowEMA[t].query_gamma_weights, variance[t].query_gamma_weights, beta1_pow_store, beta2_pow_store, beta3_pow_store, learningRate, iterationNum, ADEAMIX_ALPHA, ADEAMIX_WEIGHT_DECAY, xTotalThreads);
